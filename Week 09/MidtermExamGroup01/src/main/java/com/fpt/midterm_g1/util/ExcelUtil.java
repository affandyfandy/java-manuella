package com.fpt.midterm_g1.util;
    ...
    public static List<Product> parseProductFile(InputStream is) throws Exception {
        ...
        try (XSSFWorkbook workbook = new XSSFWorkbook(is)) {
            ...
            for (Row row : sheet) {
                ...
                BigDecimal price = getCellValueAsBigDecimal(row.getCell(1));

                Status status = getCellValueAsStatus(row.getCell(2));

                Timestamp createdTime = getCellValueAsTimestamp(row.getCell(3));
                Timestamp updatedTime = getCellValueAsTimestamp(row.getCell(4));
